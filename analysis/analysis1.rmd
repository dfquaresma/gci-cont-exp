...
print_summary_table <- function(runtime_language, nogci, gci, zero) {
  ...
  nogcierr <- (nogci %>% filter(status != 200))$request_time
  gcireq <- (gci %>% filter(status == 200))$request_time
  gcierr <- (gci %>% filter(status != 200))$request_time
  zeroreq <- (zero %>% filter(status == 200))$request_time
  zeroerr <- (zero %>% filter(status != 200))$request_time
...
read.al <- function(runtime_language, type, round) {
  p <- paste(path, runtime_language, type, "-access-e1", "r", round, ".log", sep="")
