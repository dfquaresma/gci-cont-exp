...
#COMMAND="sudo docker run -d --network=host --cpus=1.0 --cpuset-cpus=0 --memory=${CONTAINER_MEM_LIM} --memory-swap=${CONTAINER_MEM_LIM} --rm --name container-${CONTAINER_TAG} image-${CONTAINER_TAG}"
COMMAND="sudo docker run -d --network=host --cpus=1.0 --cpuset-cpus=0 --rm --name container-${CONTAINER_TAG} image-${CONTAINER_TAG}"
